...
def load_params(params_path:str)->int:
    try:
        ...
        return params["model_building"]["depth"]
    except Exception as e:
...
def train_model(x,y,depth):
    try:
        cat_features = [i for i, col in enumerate(x.columns) if x[col].dtype == 'object']
        trained_model= CatBoostRegressor(subsample= 1, min_data_in_leaf=100, learning_rate= 0.01, iterations= 2000, depth= depth, colsample_bylevel= 0.3).fit(x,y,cat_features=cat_features)
